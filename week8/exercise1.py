...
def make_filler_text_dictionary():
    ...
    for lengthy in range(3,8):
        # if type(i) == int and length >= 3:
        fullurl=url.format(leng=lengthy)
        templist =[] 
        for i in range(0,3):
...
def random_filler_text(number_of_words=200):
    ...
    mylist =[]
    dictdict =make_filler_text_dictionary()
    for i in range(0,number_of_words):
        xi = random.randint(3,7)
        xu = random.randint(0,2)
        mylist.append(dictdict[xi][xu])
    return(', '.join(mylist))
